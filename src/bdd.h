...
#ifndef __IDNI__BDD_H__
#define __IDNI__BDD_H__
...
template<typename X>
size_t std::hash<std::vector<X>>::operator()(const std::vector<X>& vec) const {
	std::hash<X> hasher;
...
template<> struct std::hash<idni::bdd_key> {
	size_t operator()(const idni::bdd_key&) const;
};
template<> struct std::hash<idni::ite_memo>{
	size_t operator()(const idni::ite_memo&) const;
};
template<> struct std::hash<std::array<idni::int_t, 2>> {
	size_t operator()(const std::array<idni::int_t, 2>&) const;
};
template<> struct std::hash<std::array<idni::bdd_ref, 2>>{
	size_t operator()(const std::array<idni::bdd_ref, 2>&) const;
};
