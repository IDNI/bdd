...
//------------------------------------------------------------------------------
// auxiliary functions
bdd_shft bdd_root(cr_spbdd_handle x) { return bdd::var(x->b); }
	...
	size_t n_vars)
{
	...
	for (size_t i = 0; i < ext_bits * n_vars; ++i)
		exvec.push_back(i > n_vars * bits);
	c = bdd::bdd_ex(c, exvec);
	return bdd_handle::get(c);
}

// ----------------------------------------------------------------------------
bdd_ref bdd::bdd_quantify(bdd_ref x, uint_t bit,
	...
	const size_t n_args)
{
	...
	size_t idx = bit / bits;
	if (x == T || x == F) return quants[idx] == quant_t::UN ? F : x;
	bdd c = bdd::get(x);
	bdd_ref h, l;
	...
	const size_t y_bitw /*, size_t x_idx = 0, size_t y_idx = 0*/)
{
	...
	if ((a_in == T) ||
		...
		if (b.h != F) c = add(pos, leq(a_in, b.h, pos, x_bitw, y_bitw
			/*, x_idx, y_idx*/), T);
		else c = add(pos, F, leq(a_in,b.l, pos, x_bitw, y_bitw
			/*, x_idx, y_idx*/));
	} else {
		if (GET_SHIFT(a_in) + (int_t)x_bitw == GET_SHIFT(b_in))
			pos = GET_SHIFT(a_in) + (y_bitw - x_bitw);
		else pos = GET_SHIFT(a_in);
		if (b.h != F) c = add(pos, leq(a.h, b.h, pos, x_bitw, y_bitw
			/*, x_idx, y_idx*/), a.l);
		else {
			if (pos-1 > bit) {
				c = add(pos, leq(a.h, b.l, bit + delta, x_bitw,
					y_bitw/*, x_idx, y_idx*/), a.l );
				c = add(pos - 1, F, c );
			}
...
bdd_ref bdd::bitwise_and(bdd_ref a_in, bdd_ref b_in) {
	...
	if (GET_SHIFT(a_in) > GET_SHIFT(b_in) + 1 && GET_SHIFT(b_in) != 0)
		a.h = a_in, a.l = a_in, pos = GET_SHIFT(b_in) + 1;
	else if (GET_SHIFT(a_in) + 2 < GET_SHIFT(b_in) && GET_SHIFT(a_in) != 0)
		b.h = b_in, b.l = b_in, pos = GET_SHIFT(a_in) + 2;
	else if (GET_SHIFT(a_in) != 0)  pos = GET_SHIFT(a_in) + 2;
	else pos = GET_SHIFT(b_in) + 1;
	bdd_ref c = add(pos, bitwise_and(a.h, b.h), bdd_or(
		bdd_or(bitwise_and(a.h,  b.l), bitwise_and(a.l, b.h)),
		bitwise_and(a.l, b.l)));
	return c;
}
...
bdd_ref bdd::bitwise_or(bdd_ref a_in, bdd_ref b_in) {
	...
	bdd_ref c = add(pos, bdd_or(
		bdd_or(bitwise_or(a.h, b.l), bitwise_or(a.l, b.h)),
...
bdd_ref bdd::bitwise_not(bdd_ref a_in) { return a_in; } // TODO: implement
...
bdd_ref bdd::adder(bdd_ref a_in, bdd_ref b_in, bool carry, size_t bit) {
	...
	if      (av > bv + 1 && bv != 0) a.h = a_in, a.l = a_in, pos = bv + 1;
	else if (av + 2 < bv && av != 0) b.h = b_in, b.l = b_in, pos = av + 2;
	else                                                     pos = av + 2;
	if (pos > (bit + 1) * 3) {
		size_t delta = (pos / 3) - 1;
		return carry ? add(pos-3,bdd_or(adder(a_in, b_in,true, delta),
				                adder(a_in, b_in,false,delta)),
				         adder(a_in, b_in, true, delta))
			     : add(pos-3,adder(a_in, b_in, false, delta),
				         bdd_or(adder(a_in, b_in,true, delta),
				                adder(a_in, b_in,false,delta)));
	}
	return carry ? add(pos, bdd_or(adder(a.h, b.h, true,  bit + 1),
		                       adder(a.l, b.l, false, bit + 1)),
		                bdd_or(adder(a.h, b.l, true,  bit + 1),
		                       adder(a.l, b.h, true,  bit + 1)))
		     : add(pos, bdd_or(adder(a.h, b.l, false, bit + 1),
		                       adder(a.l, b.h, false, bit + 1)),
		                bdd_or(adder(a.h, b.h, true,  bit + 1),
		                       adder(a.l, b.l, false, bit + 1)));
}
// ----------------------------------------------------------------------------
// Over bdds MULT
...
bdd_ref bdd::merge_pathX(size_t i, size_t bits, bool carry, size_t n_args,
	size_t depth, t_pathv& path_a, t_pathv& path_b, t_pathv& pathX_a,
	t_pathv& pathX_b)
{
	bdd_ref c;
	if (!carry) {
		std::map<size_t,bdd_ref>::iterator it = covered_cf.find(i);
		if (it != covered_cf.end() && pathX_a.size() == 0 &&
			pathX_b.size() == 0) c = covered_cf[i];
		else c = solve_path(i, bits, false, n_args, depth, path_a,
			path_b, pathX_a, pathX_b), covered_cf[i] = c;
	} else {
		std::map<size_t,bdd_ref>::iterator it = covered_ct.find(i);
		if (it != covered_ct.end() && pathX_a.size() == 0 &&
			pathX_b.size() == 0) c = covered_ct[i];
		else c = solve_path(i, bits, true, n_args, depth, path_a,
			path_b, pathX_a, pathX_b), covered_ct[i] = c;
	}
	return c;
}

bdd_ref bdd::solve_pathXL(size_t i, size_t bits, bool carry, size_t n_args,
	size_t depth, t_pathv& path_a, t_pathv& path_b, t_pathv& pathX_a,
	t_pathv& pathX_b)
{
	...
	if (path_a[bits - i - 1] == X && path_b[bits - i - 1] == L) {
		if (pathX_b.size() == 0) {
			...
			pathX_a.push_back(L);
			bdd_ref c0;
			c0 = solve_pathXL(i + 1,bits, false, n_args, depth,
				path_a, path_b, pathX_a, pathX_b);
			...
			pathX_a.push_back(H);
			bdd_ref c1;
			if (!carry) c1 = solve_pathXL(i+1, bits, false, n_args,
					depth, path_a, path_b, pathX_a,pathX_b),
				c = add(pos, c1, c0);
			else c1 = solve_pathXL(i + 1, bits, true, n_args,
					depth, path_a, path_b, pathX_a,pathX_b),
				c = add(pos, c0, c1);
		} else {
...
#endif
			bdd_ref c0;
			if (!carry) c0 = solve_pathXL(i+1, bits, false, n_args,
					depth, path_a, path_b, pathX_a,pathX_b),
				c = aux==L ? add(pos, F, c0) : add(pos, c0, F);
			else {
				if (aux == L) c0 = solve_pathXL(i + 1, bits,
						false, n_args, depth, path_a,
						path_b, pathX_a, pathX_b),
					c = add(pos, c0, F);
				else c0 = solve_pathXL(i + 1,bits, true, n_args,
						depth, path_a, path_b, pathX_a,
						pathX_b), c = add(pos, F, c0);
			}
		}
	} else {
		c = merge_pathX(i, bits, carry, n_args, depth, path_a, path_b,
							pathX_a, pathX_b);
...
bdd_ref bdd::solve_pathLX(size_t i, size_t bits, bool carry, size_t n_args,
	size_t depth, t_pathv& path_a, t_pathv& path_b, t_pathv& pathX_a,
	t_pathv& pathX_b)
{
	int_t pos = i * n_args + 3;
	...
	if (path_a[bits - i - 1] == L && path_b[bits - i - 1] == X) {
		if (pathX_a.size() == 0) {
			...
			pathX_b.push_back(L);
			bdd_ref c0;
			c0 = solve_pathLX(i + 1, bits, false, n_args, depth,
					path_a, path_b, pathX_a, pathX_b);
			pathX_b = aux_pathX_b;
#ifdef VERBOSE
			cout << " ---solve LX pos = " << pos << "\n";
			...
			pathX_b.push_back(H);
			bdd_ref c1;
			if (!carry) c1 = solve_pathLX(i+1, bits, false, n_args,
					depth, path_a, path_b, pathX_a,pathX_b),
				c = add(pos, c1, c0);
			else c1 = solve_pathLX(i + 1, bits, true, n_args, depth,
					path_a, path_b, pathX_a, pathX_b),
				c = add(pos, c0, c1);
		} else {
...
#endif
			bdd_ref c0;
			if (!carry) c0 = solve_pathLX(i+1, bits, false, n_args,
					depth, path_a, path_b, pathX_a,pathX_b),
				c = aux==L ? add(pos, F, c0) : add(pos, c0, F);
			else {
				if (aux == L) c0 = solve_pathLX(i+1,bits, false,
					n_args, depth, path_a, path_b, pathX_a,
					pathX_b), c = add(pos, c0, F);
				else c0 = solve_pathLX(i+1, bits, true,	n_args,
					depth, path_a, path_b, pathX_a,pathX_b),
					c = add(pos, F, c0);
			}
		}
...
bdd_ref bdd::solve_pathXH(size_t i, size_t bits, bool carry, size_t n_args,
	size_t depth, t_pathv& path_a, t_pathv& path_b, t_pathv& pathX_a,
	t_pathv& pathX_b)
{
	int_t pos = i * n_args + 3;
	...
	if (path_a[bits - i - 1] == X && path_b[bits - i - 1] == H) {
		if (pathX_b.size() == 0) {
			...
			pathX_a.push_back(L);
			bdd_ref c0, c1;
			c0 = solve_pathXH(i + 1, bits, carry, n_args, depth,
				path_a, path_b, pathX_a, pathX_b);
			pathX_a = aux_pathX_a;
#ifdef VERBOSE
			cout << " ---solve XH pos = " << pos << "\n";
#endif
			pathX_a.push_back(H);
			if (!carry) c1 = solve_pathXH(i+1, bits, true, n_args,
					depth, path_a, path_b, pathX_a,pathX_b),
				c = add(pos, c0, c1);
			else c1 = solve_pathXH(i + 1, bits, true, n_args, depth,
					path_a, path_b, pathX_a, pathX_b),
				c = add(pos, c1, c0);
		} else {
			t_path aux = pathX_b.front();
			...
			bdd_ref c0;
			if (!carry) {
				if (aux == L) c0 = solve_pathXH(i+1, bits,false,
					n_args, depth, path_a, path_b, pathX_a,
					pathX_b), c = add(pos, c0, F);
				else c0 = solve_pathXH(i+1, bits, true, n_args,
					depth, path_a, path_b, pathX_a,pathX_b),
					c = add(pos, F, c0);
			} else {
				if (aux == L) c0 = solve_pathXH(i+1, bits, true,
					n_args, depth, path_a, path_b, pathX_a,
					pathX_b), c = add(pos, F, c0);
				else c0 = solve_pathXH(i+1, bits, true, n_args,
					depth, path_a, path_b, pathX_a,pathX_b),
	...
	} else c = merge_pathX(i, bits, carry, n_args, depth, path_a, path_b,
							pathX_a, pathX_b);
		//c = solve_path(i,bits, carry, n_args, depth, path_a, path_b, pathX_a, pathX_b);
	return c;
}

bdd_ref bdd::solve_pathHX(size_t i, size_t bits, bool carry, size_t n_args,
	size_t depth, t_pathv& path_a, t_pathv& path_b, t_pathv& pathX_a,
	t_pathv& pathX_b)
{
	int_t pos = i * n_args + 3;
	...
	if (path_a[bits - i - 1] == H && path_b[bits - i - 1] == X) {
		if (pathX_a.size() == 0) {
			...
			pathX_b.push_back(L);
			bdd_ref c0, c1;
			c0 = solve_pathHX(i + 1, bits, carry, n_args, depth,
					path_a, path_b, pathX_a, pathX_b);
...
#endif
			pathX_b.push_back(H);
			if (!carry) c1 = solve_pathHX(i + 1, bits, true, n_args,
					depth, path_a, path_b, pathX_a,pathX_b),
				c = add(pos, c0, c1);
			else c1 = solve_pathHX(i + 1, bits, true, n_args, depth,
					path_a, path_b, pathX_a, pathX_b),
				c = add(pos, c1, c0);
		} else {
			t_path aux = pathX_a.front();
			...
			bdd_ref c0;
			if (!carry) {
				if (aux == L) c0 = solve_pathHX(i+1, bits,false,
					n_args, depth, path_a, path_b, pathX_a,
					pathX_b), c = add(pos, c0, F);
				else c0 = solve_pathHX(i+1, bits, true, n_args,
					depth, path_a, path_b, pathX_a,pathX_b),
					c = add(pos, F, c0);
			} else {
				if (aux == L) c0 = solve_pathXH(i+1, bits, true,
...
bdd_ref bdd::solve_pathXX(size_t i, size_t bits, bool carry, size_t n_args,
	size_t depth, t_pathv& path_a, t_pathv& path_b, t_pathv& pathX_a,
	t_pathv& pathX_b)
{
	bdd_ref c = T;
	...
	if (path_a[bits - i - 1] == X && path_b[bits - i - 1] == X) {
		if (pathX_a.size() == 0 && pathX_b.size() == 0) {
			while (path_a[bits-i-1] == X && path_b[bits-i-1] == X)
				i++;
			//t_pathv aux_pathX_a = pathX_a;
			//t_pathv aux_pathX_b = pathX_b;
			bdd_ref c0 = solve_path(i, bits, false, n_args, depth,
					path_a, path_b, pathX_a, pathX_b);
			//int_t c1 = solve_path(i,bits, true, n_args, depth, path_a, path_b, aux_pathX_a, aux_pathX_b);
			bdd_ref c1 = solve_path(i, bits, true, n_args, depth,
					path_a, path_b, pathX_a, pathX_b);
			...
			t_pathv aux_pathX_a = pathX_a, aux_pathX_b = pathX_b;
			if (pathX_a.size() > 0) aux = pathX_a.front(),
				pathX_a.push_back(L),
				pathX_a.erase(pathX_a.begin());
			else aux = pathX_b.front(), pathX_b.push_back(L),
				pathX_b.erase(pathX_b.begin());
			if (!carry) {
				if (aux == L) c0 = solve_pathXX(i+1, bits,false,
					n_args, depth, path_a, path_b, pathX_a,
					pathX_b), cl = add(pos, F, c0);
				 else c1 = solve_pathXX(i+1, bits, false,n_args,
				 	depth, path_a, path_b, pathX_a,pathX_b),
					cl = add(pos, c1, F);
			} else {
				if (aux == L) c1 = solve_pathXX(i+1, bits,false,
					n_args, depth, path_a, path_b, pathX_a,
					pathX_b), cl = add(pos, c1, F);
				else c0 = solve_pathXX(i+1, bits, true, n_args,
					depth, path_a, path_b, pathX_a,pathX_b),
					cl = add(pos, F, c0);
			}
			pathX_a = aux_pathX_a, pathX_b = aux_pathX_b;
			if (pathX_a.size() > 0) aux = pathX_a.front(),
				pathX_a.push_back(H),
				pathX_a.erase(pathX_a.begin());
			else aux = pathX_b.front(), pathX_b.push_back(H),
				pathX_b.erase(pathX_b.begin());
			if (!carry) {
				if (aux == L) c0 = solve_pathXX(i+1, bits,false,
					n_args, depth, path_a, path_b, pathX_a,
					pathX_b), ch = add(pos, c0, F);
				else c0 = solve_pathXX(i+1, bits, true, n_args,
					depth, path_a, path_b, pathX_a,pathX_b),
					ch = add(pos, F, c0);
			} else {
				if (aux == L) c0 = solve_pathXX(i+1, bits, true,
					n_args, depth, path_a, path_b, pathX_a,
					pathX_b), ch = add(pos, F, c0);
				else c0 = solve_pathXX(i+1, bits, true, n_args,
					depth, path_a, path_b, pathX_a,pathX_b),
					ch = add(pos, c0, F);
			}
	...
	} else c = solve_path(i, bits, carry, n_args, depth, path_a, path_b,
							pathX_a, pathX_b);
	return c;
}

bdd_ref bdd::solve_path(size_t i, size_t bits, bool carry, size_t n_args,
	size_t depth, t_pathv& path_a, t_pathv& path_b, t_pathv& pathX_a,
	t_pathv& pathX_b)
{
	if (i == bits) return T;
	...
	if (!carry) {
		if(path_a[bits - i - 1] == L && path_b[bits - i - 1] == L)
			c = add(pos, F, solve_path(i+1, bits,false,n_args,depth,
					path_a, path_b, pathX_a, pathX_b));
		else if ((path_a[bits - i - 1] == L && path_b[bits-i-1] == H) ||
				(path_a[bits-i-1] == H && path_b[bits-i-1] ==L))
			c = add(pos, solve_path(i+1, bits, false, n_args, depth,
					path_a, path_b, pathX_a, pathX_b), F);
		else if (path_a[bits - i - 1] == H && path_b[bits - i - 1] == H)
			c = add(pos, F, solve_path(i+1, bits, true,n_args,depth,
					path_a, path_b, pathX_a, pathX_b));
		else if (path_a[bits - i - 1] == X && path_b[bits - i - 1] == H)
			c = solve_pathXH(i, bits, false, n_args, depth,
					path_a, path_b, pathX_a, pathX_b);
		else if (path_a[bits - i - 1] == H && path_b[bits - i - 1] == X)
			c = solve_pathHX(i, bits, false, n_args, depth,
					path_a, path_b, pathX_a, pathX_b);
		else if (path_a[bits - i - 1] == X && path_b[bits - i - 1] == L)
			c = solve_pathXL(i, bits, false, n_args, depth,
					path_a, path_b, pathX_a, pathX_b);
		else if (path_a[bits - i - 1] == L && path_b[bits - i - 1] == X)
			c = solve_pathLX(i, bits, false, n_args, depth,
					path_a, path_b, pathX_a, pathX_b);
		else if(path_a[bits - i - 1] == X && path_b[bits - i - 1] == X)
			c = solve_pathXX(i, bits, false, n_args, depth,
					path_a, path_b, pathX_a, pathX_b);
		else cout << " --- ERROR" << "\n";
	} else {
		if(path_a[bits - i - 1] == L && path_b[bits - i - 1] == L)
			...
			c = add(pos, solve_path(i + 1,bits, true, n_args, depth,
					path_a, path_b, pathX_a, pathX_b), F);
		else if (path_a[bits - i - 1] == X && path_b[bits - i - 1] == H)
			c = solve_pathXH(i,bits, true, n_args, depth,
					path_a, path_b, pathX_a, pathX_b);
		else if	(path_a[bits - i - 1] == H && path_b[bits - i - 1] == X)
			c = solve_pathHX(i,bits, true, n_args, depth,
					path_a, path_b, pathX_a, pathX_b);
		else if (path_a[bits - i - 1] == X && path_b[bits - i - 1] == L)
			c = solve_pathXL(i,bits, true, n_args, depth,
					path_a, path_b, pathX_a, pathX_b);
		else if (path_a[bits - i - 1] == L && path_b[bits - i - 1] == X)
			c = solve_pathLX(i,bits, true, n_args, depth,
					path_a, path_b, pathX_a, pathX_b);
		else if(path_a[bits - i - 1] == X && path_b[bits - i - 1] == X)
			c = solve_pathXX(i, bits, true, n_args, depth,
					path_a, path_b, pathX_a, pathX_b);
		else cout << " --- ERROR" << "\n";
	}
...
bool bdd::bdd_next_path(std::vector<bdd_ref>& a, int_t& i, int_t& bit,
	t_pathv& path, size_t bits, size_t n_args)
{
	...
	bool goh = false;
	while (!done) {
		if (path[bit] == U) done = true;
		else if (path[bit] == L && bdd::hi(a[i]) != F)
			done = true, goh = true;
		else if (path[bit] != X) path[bit--] = U, a.pop_back(), i--;
		else path[bit--] = U;
		if (bit < 0) return false;
	}
	if (bdd::hi(a[i]) == F && bdd::lo(a[i]) == F) return false;
	done = false;
	while (!done) {
		if (bdd::lo(a[i]) != F && !goh) {
			if (bdd::lo(a[i]) == T) {
				size_t delta = bits - bit;
				done = true, path[bit] = L;
				if (delta > 1) for (size_t j=1; j < delta; j++)
							path[bit + j] = X;
			} else {
				a.push_back(bdd::lo(a[i])); //bdd::lo(a[i]);
				size_t delta = (GET_SHIFT(a[i+1]) -
						GET_SHIFT(a[i])) / n_args;
				path[bit] = L;
				if (delta > 1) for(size_t j = 1; j < delta; j++)
							path[bit + j] = X;
				i++, bit = bit + delta;
	...
	std::vector<t_pathv>& aux_path_a, std::vector<t_pathv>& aux_path_b)
{
	...
	if (aux_path_a.size() != 0) next_path_a = aux_path_a.front(),
		aux_path_a.erase(aux_path_a.begin());
	if (aux_path_b.size() != 0) next_path_b = aux_path_b.front(),
		aux_path_b.erase(aux_path_b.begin());
	for (size_t i = 0; i < bits ; i++ ) {
		if (next_path_a[i] == X) count_a = count_a * 2;
		if (next_path_b[i] == X) count_b = count_b * 2;
	}
	if (count_a == count_b) {
	...
	if (count_a < count_b) {
		...
		while (count_b > count_a) {
			if (next_path_b[i] == X)
				next_path_b[i] = L, aux_path[i] = H,
				aux_path_b.insert(aux_path_b.begin(), aux_path),
				aux_path[i] = L, count_b = count_b >> 1;
			i++;
		}
		return 1;
	} else {
		...
		while (count_a > count_b) {
			if (next_path_a[i] == X)
				next_path_a[i] = L, aux_path[i] = H,
				aux_path_a.insert(aux_path_a.begin(), aux_path),
				aux_path[i] = L, count_a = count_a >> 1;
			i++;
		}
		return -1;
	}
...
{
	t_pathv path_a(bits, U), path_b(bits, U),
		next_path_a, next_path_b, pathX_a, pathX_b;
	std::vector<t_pathv> aux_path_a, aux_path_b;
	std::vector<bdd_ref> a, b;
	a.push_back(a_in), b.push_back(b_in);
	int_t a_i = 0, a_bit = 0;
	int_t b_i = 0, b_bit = 0;
	bool a_path_vld = true, b_path_vld = true;
	bdd_ref aux;
	a_path_vld = bdd_next_path(a, a_i, a_bit, path_a, bits, n_args);
	b_path_vld = bdd_next_path(b, b_i, b_bit, path_b, bits, n_args);
	next_path_a = path_a, next_path_b = path_b;
	while (a_path_vld && b_path_vld) {
		int_t bal = balance_paths(next_path_a, next_path_b, bits,
			aux_path_a, aux_path_b);
		if (a_path_vld && b_path_vld) {
			aux = solve_path(0, bits, false, n_args, depth,
				next_path_a, next_path_b, pathX_a, pathX_b);
			if (bal == 0) a_path_vld = bdd_next_path(
					a, a_i, a_bit, path_a, bits, n_args),
				b_path_vld = bdd_next_path(
					b, b_i, b_bit, path_b, bits, n_args),
				next_path_a = path_a, next_path_b = path_b;
			else if (bal == 1) a_path_vld = bdd_next_path(
					a, a_i, a_bit, path_a, bits, n_args),
				next_path_a = path_a;
			else b_path_vld = bdd_next_path(
					b, b_i, b_bit, path_b, bits, n_args),
				next_path_b = path_b;
			c = bdd_or(c, aux);
		}
	}
	covered_cf.clear(), covered_ct.clear();
	return;
}
...
bdd_ref bdd::shr(bdd_ref a_in, size_t arg, size_t bits, size_t n_args) {
	...
	bools exvec;
	for (size_t i = 0; i < tbits; ++i) exvec.push_back(i == base);
...
bdd_ref bdd::bdd_shift(bdd_ref a_in, bdd_shft amt) {
	return PLUS_SHIFT(a_in, amt);
}
	...
	size_t n_args)
{
	...
	else b_aux = bdd_permute(b_aux, perm1, memos_perm[perm1]),
		//cout << "##[baux inv]:" << endl;
		//out(cout, b_aux);
		//cout <<endl<<endl;
...
bdd_ref bdd::zero(size_t arg, size_t bits, size_t n_args) {
	...
	int_t pos;
	for (size_t i = 0; i < bits; i++)
		pos = n_args * (bits - 1 - i) + arg, aux = add(pos, F, aux);
	return aux;
}
...
void bdd::mult_dfs(bdd_ref a_in, bdd_ref b_in, bdd_ref *accs, size_t depth,
	size_t bits, size_t n_args, bdd_ref& c)
{
	if (depth == bits) return c = bdd_or(c, accs[depth-1]), (void) 0;
	bdd_shft pos = n_args * depth + 1;
	bdd a = get(a_in);
	if (GET_SHIFT(a_in) > pos) {a.h = a_in, a.l = a_in;}
	if (a.l != F) accs[depth] = depth == 0
			? zero(3, bits, n_args) : accs[depth - 1],
		mult_dfs(a.l, b_in, accs, depth + 1, bits, n_args, c);
	//XXX improve to avoid call to is_zero once it has been false
	if (a.h != F) accs[depth] = depth == 0
			? copy_arg2arg(b_in, 1, 2, bits, n_args)
			: adder_accs(b_in, accs[depth-1], depth, bits, n_args),
		mult_dfs(a.h, b_in, accs, depth + 1, bits, n_args, c);
}

bdd_ref bdd::bdd_xor(bdd_ref x, bdd_ref y) {
	return bdd_ite(x, FLIP_INV_OUT(y), y);
}
