...
int main() {
	...
	cout << endl << "Info: Cache size before conducting shifted "
		"computations is " << orig_ite_cache_size << "." << endl;
	...
	if(ite_cache_size <= orig_ite_cache_size) {
		cout << endl << "Error: Unshifted BDD computations caused the "
			"cache to shrink by " <<
			(orig_ite_cache_size - ite_cache_size) << "." << endl;
		return 4;
	} else {
		cout << endl << "Info: Unshifted BDD computations caused the "
			"cache to grow by " <<
			(ite_cache_size - orig_ite_cache_size) << "." << endl;
	}
	...
	for(int shift = 1; shift < 10; shift++) {
		...
		for(const spbdd_handle &func1 : func_set) {
			for(const spbdd_handle &func2 : func_set) {
				...
				func_set2.insert(bdd_shift(func1, shift) &&
					bdd_shift(func2, shift));
				func_set2.insert(bdd_shift(func1, shift) ||
					bdd_shift(func2, shift));
				func_set2.insert(bdd_shift(func1, shift) %
					bdd_shift(func2, shift));
				for(const spbdd_handle &func3 : func_set) {
					func_set2.insert(bdd_ite(
						bdd_shift(func1, shift),
						bdd_shift(func2, shift),
						bdd_shift(func3, shift)));
				}
			}
		}
		if(func_set.size() != func_set2.size()) {
			cout << endl << "Error: Boolean function set generated "
				"by set shifted by " << shift << " not "
				"equinumerous to that generated by original "
				"set." << endl;
			return 2;
		} else if(bdd::get_ite_cache_size() != ite_cache_size) {
			cout << endl << "Error: Cache not being used to short-"
				"circuit BDD computations shifted by " << shift
				<< "." << endl;
			return 3;
		} else {
			// Control flow passing through here implies that BDD computations are most-likely short-circuited
			cout << endl << "Success: BDD computations shifted by "
				<< shift << " were most-likely short-circuited "
				"using the cache." << endl;
		}
	}
	return 0;
}
