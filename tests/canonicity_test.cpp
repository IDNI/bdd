...
void generate_functions(int arity, unordered_set<spbdd_handle> &func_set,
	int offset = 0)
{
	if(arity == 0) {
		...
		for(const spbdd_handle &func1 : func_set) {
			for(const spbdd_handle &func2 : func_set) {
				pending.insert(bdd_ite_var(offset + arity,
					func1, func2));
			}
		}
...
int main() {
	...
	for(const spbdd_handle &func1 : func_set) {
		for(const spbdd_handle &func2 : func_set) {
			...
			if(func_set.find(func1 && func2) == func_set.end()) {
				cout << endl << "Error: Boolean function set "
					"not closed under conjunction." << endl;
				return 1;
			} else if(func_set.find(func1||func2)==func_set.end()) {
				cout << endl << "Error: Boolean function set "
					"not closed under disjunction." << endl;
				return 1;
			} else if(func_set.find(func1%func2)==func_set.end()) {
				cout << endl << "Error: Boolean function set "
					"not closed under relative "
					"complementation." << endl;
				return 1;
			} else cout << ".";
			// Check closure under ternary operations
			for(const spbdd_handle &func3 : func_set) {
				if(func_set.find(bdd_ite(func1, func2, func3))
					== func_set.end())
				{
					cout << endl << "Error: Boolean "
						"function set not closed under "
	...
	cout << endl << "Success: Boolean function set closed under chosen "
		"operations." << endl;
	...
	for(int shift = 1; shift < 10; shift++) {
		...
		if(func_set == func_set2) {
			cout << endl << "Error: Shifted function set cannot be "
				"identical to the original." << endl;
			return 2;
		} else if(func_bdd_set != func_bdd_set2) {
			cout << endl << "Error: Shifted function set should "
				"have BDDs identical to the original's."<< endl;
			return 3;
		}
	}
	// Control flow passing through here implies shifts share BDDs
	cout << endl << "Success: Boolean function set is represented by same "
		"BDDs despite variable shifts." << endl;
	...
	for(const spbdd_handle &func : func_set) {
		func_out_inv_split[make_tuple((int) GET_BDD_ID(func->b),
			(int) GET_SHIFT(func->b), (bool) GET_INV_INP(func->b))
		].push_back(GET_INV_OUT(func->b));
	}
	if(2 * func_out_inv_split.size() != func_set.size()) {
		...
		cout << endl << "Error: Output inversion attribute does not "
			"equipartition function set into two sets." << endl;
		return 4;
	} else {
		...
		for(const auto &[func_part, inv_outs] : func_out_inv_split) {
			if(inv_outs.size() < 2) {
				cout << endl << "Error: At least one function-"
					"inverse pair is not represented with "
					"the same BDD." << endl;
				return 5;
			} else if(inv_outs.size() > 2) {
				cout << endl << "Error: At least one function "
					"is over-represented in the function "
					"set." << endl;
				return 5;
			} else if (inv_outs[0] == inv_outs[1]) {
				cout << endl << "Error 1: At least one function"
					"-inverse pair is not represented with "
					"the same BDD." << endl;
				cout << endl << "Error 2: At least one function"
					" is over-represented in the function "
					"set." << endl;
				return 5;
			}
	...
	cout << endl << "Success: Boolean function set is equipartitioned into "
		"two sets by the output inversion attribute." << endl;
	return 0;
}
